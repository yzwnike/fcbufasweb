  ...
  try {
    ...
    async function pickRandomCardIdByGroup(conn: any, group: 'BASE_OG' | 'LEGEND' | 'RARE' | 'ELITE'): Promise<Card | null> {
      ...
      if (group === 'LEGEND') {
        ...
        if (c) return c;
        // Fallbacks si no hay POTM en la BD (excluir Jorge y Juanete COMEBACK_HERO de sobres normales)
const c2 = await pick(`SELECT c.* FROM cards c JOIN players p ON c.player_id = p.id WHERE c.special_type IN ('ASSIST_ENGINE','RATING_RELOAD','MARKET_MASTER','COMEBACK_HERO','TEAM_OF_THE_WEEK','NOM_POTM') AND NOT (p.card_asset_basename IN ('jorge','juanete') AND c.special_type = 'COMEBACK_HERO') ORDER BY RANDOM() LIMIT 1`);
        if (c2) return c2;
      ...
      if (group === 'ELITE') {
        ...
        // COMEBACK_HERO (Jorge y Juanete) NUNCA salen en sobres normales (solo EVENTO/ELITE de logros/SBC)
        const c = await pick(`SELECT c.* FROM cards c JOIN players p ON c.player_id = p.id WHERE c.special_type IN ('MARKET_MASTER','RATING_RELOAD','ASSIST_ENGINE') AND NOT (p.card_asset_basename IN ('jorge','juanete') AND c.special_type = 'COMEBACK_HERO') ORDER BY RANDOM() LIMIT 1`);
        if (c) return c;
        // Fallbacks seguros
